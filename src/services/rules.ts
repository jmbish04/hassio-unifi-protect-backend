...
export class RulesEngine {
	evaluateRules({ haStates, cam, vision }: { haStates: HAStates; cam: string; vision: VisionAnalysis | null }): RuleResult[] {
		// read HA states
	...
	computeBooleans(obs: Observation[], ha: HAStates): Record<string, boolean> {
		const carDoor = obs.some((o) => o.rule === 'car_and_door_open' && (o.result === 'warn' || o.result === 'fail'));
